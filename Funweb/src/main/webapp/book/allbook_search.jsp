...
int listCount = bookDAO.selectSearchListCount(search);
...
ArrayList<BookBean> bookList = bookDAO.selectSearchBookList(pageNum, listLimit, search);
...
<body>
	<div id="wrap">
		...
		<nav id="sub_menu">
			<ul>
				<li><a href="bestSeller.jsp">All Books</a></li>
		...
		<article>
         ...
         <table id = "list">
            <tr onclick="location.href='bestSeller_content.jsp?num=<%=book.getNum() %>&page=<%=pageNum %>'">
               <td>
         ...
         <%} %>
			...
			<div id="table_search">
				<form action="allbook_search.jsp" method="get">
					<input type="text" name="search" class="input_box"
						placeholder="검색어 입력" value="<%=search%>"> 
			...
			<div id="page_control">
				...
				%>
				<a href="bestSeller.jsp?page=<%=pageNum - 1%>">Prev</a>
				<%
				...
				%>
				<a href="bestSeller.jsp?page=<%=i%>"><%=i%></a>
				<%
				}
				%>
				<%
				...
				%>
				<a href="bestSeller.jsp?page=<%=pageNum + 1%>">Next</a>
				<%
				} else { // 이전페이지가 존재하지 않을 경우
				%>
				&nbsp;Next
